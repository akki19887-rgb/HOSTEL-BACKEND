...
def _file_belongs_to_property(path, property_id):
    ...
    if path.startswith('payoutDocs/'):
        parts = path.split('/')
        return len(parts) >= 2 and parts[1] == property_id

    # (collection, field) pairs that can legitimately hold one of the other prefixes
    lookups = []
    if path.startswith('guestAadharDocs/'):
        lookups = [('guestRegistrations', 'aadharPhotoUrl'),
                   ('guestRegistrations', 'aadharPhotoBackUrl'),
                   ('bookings', 'idPhotoPath')]          # walk-in guests
    elif path.startswith('guestRegistrationPdfs/'):
        lookups = [('guestRegistrations', 'pdfUrl')]
    elif path.startswith('upiPaymentProofs/'):
        lookups = [('bookings', 'screenshotUrl')]
    else:
        return False

    for coll, field in lookups:
        try:
            for d in firestore_db.collection(coll).where(field, '==', path).limit(3).stream():
                if (d.to_dict() or {}).get('propertyId') == property_id:
                    return True
        except Exception as e:
            # A missing index must never turn into "yes". Fail shut.
            print(f"secure-file ownership check failed on {coll}.{field}: {e}")
            return False
    return False
...
def secure_file():
    ...
    if not is_admin:
        ...
        # Owning the property you named is not the same as the file belonging to it. Without
        # this second check an owner could name their own hostel and ask for another hostel's
        # guest ID photo.
        if not _file_belongs_to_property(path, property_id):
            return jsonify({"error": "You are not authorised to view this file."}), 403
